...
bool JOIN::make_aggr_tables_info()
{
  ...
  */
  ...
  if (unit->lim.is_with_ties())
  {
    if (alloc_order_fields(this, order, order_count_for_with_ties))
      DBUG_RETURN(true);
  }
	...
	if (end_of_records)
	  DBUG_RETURN(NESTED_LOOP_OK);
            ...
            join->do_send_rows)
        {
          ...
          if (!join->unit->lim.is_with_ties() ||
              idx < (int)join->order_count_for_with_ties)
          {
            if (!(join->select_options & OPTION_FOUND_ROWS))
              DBUG_RETURN(NESTED_LOOP_QUERY_LIMIT); // Abort nicely
            join->do_send_rows= 0;
...
/**
  ...
  SIDE EFFECT:
  For all the first items in the list that match, the sort direction of the
  group is set to the same as order. The direction of the group does not
