...
typedef bool (*set_limit_callback)(THD *thd, SELECT_LEX_UNIT *unit,
                                ha_rows lim);
typedef bool (*const_probe)(Item *item);
...
/**
   Test if an item as an inexpensive const item (safe to evalute once early)

   Used as a callback for process_direct_rownum_comparison(), but can be
   used by anyone.
*/
...
/*
  Set a limit for the select based on ROWNUM usage.
