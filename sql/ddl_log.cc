  ...
  case DDL_LOG_ALTER_TABLE_ACTION:
  {
    ...
    case DDL_ALTER_TABLE_PHASE_INPLACE:
    {
      ...
      */
      ...
      if (!access(from_path, F_OK))             // Does #sql-alter.. exists?
      {
        ...
        */
        to_path[length - reg_ext_length]= 0;  // Remove .frm
        if (!hton->check_version ||
    ...
    case DDL_ALTER_TABLE_PHASE_COPIED:
    {
      ...
      */
      ...
      if (likely(org_hton))
      {
        error= org_hton->drop_table(hton, from_path);
        if (non_existing_table_error(error))
          error= 0;
      }
  ...
  for (i= 1; i <= global_ddl_log.num_entries; i++)
  {
    ...
    if (ddl_log_entry.entry_type == DDL_LOG_EXECUTE_CODE)
    {
      ...
      */
      ...
      if (ddl_log_entry.unique_id >= DDL_LOG_MAX_RETRY)
      {
        sql_print_error("DDL_LOG: Aborting executing entry %u after %llud "
