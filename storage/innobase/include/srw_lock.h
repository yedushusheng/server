...
public:
  ...
  {
    uint32_t l= 0;
    return lock.compare_exchange_strong(l, HOLDER,
  ...
  void wr_unlock()
  {
    const uint32_t l= lock.fetch_and(~HOLDER, std::memory_order_release);
    if (l != HOLDER)
    {
...
public:
  ...
  /** Wait for readers!=l|WRITER */
  void wr_wait(uint32_t l);
...
# endif /* !DBUG_OFF */
  ...
  {
    uint32_t l= 0;
    while (!readers.compare_exchange_weak(l, l + 1,
                                          std::memory_order_acquire,
                                          std::memory_order_relaxed))
      if (l & WRITER)
        return false;
  ...
  {
    writer.wr_lock();
    if (uint32_t l= readers.fetch_or(WRITER, std::memory_order_acquire))
      wr_wait(l);
  }
  ...
  void rd_unlock()
  {
    uint32_t l= readers.fetch_sub(1, std::memory_order_release);
    ut_ad(~WRITER & l);
    if (l == WRITER + 1)
      wake();
  }
